...
const typeColors: Record<string, { bg: string; text: string; dot: string }> = {
  "Inquiry Experiment": { bg: "bg-blue-100", text: "text-blue-600", dot: "bg-blue-400" },
  "3D Resource":        { bg: "bg-purple-100", text: "text-purple-600", dot: "bg-purple-400" },
  "Video Resource":     { bg: "bg-red-100", text: "text-red-600", dot: "bg-red-400" },
  "Interactive Courseware": { bg: "bg-green-100", text: "text-green-600", dot: "bg-green-400" },
  "Study Guide":        { bg: "bg-orange-100", text: "text-orange-600", dot: "bg-orange-400" },
};

export const ExperimentCard: React.FC<ExperimentCardProps> = ({ experiment, onOpen }) => {
  ...
  const typeStyle = typeColors[experiment.type] || { bg: "bg-gray-100", text: "text-gray-600", dot: "bg-gray-400" };

  return (
    <div
      className={`group bg-white rounded-xl border overflow-hidden cursor-pointer transition-all duration-200 ${
        hovered
          ? "border-blue-300 shadow-lg shadow-blue-100/60 -translate-y-0.5"
    ...
    >
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-slate-100 to-blue-50" style={{ aspectRatio: "16/9" }}>
        {!imageError ? (
          ...
          <div className="w-full h-full flex flex-col items-center justify-center gap-2">
            <svg width="36" height="36" viewBox="0 0 24 24" fill="none" className="text-gray-300">
        ...
        {/* Hover overlay */}
        <div className={`absolute inset-0 bg-gradient-to-t from-black/60 via-black/30 to-transparent flex flex-col items-center justify-center gap-2 transition-opacity duration-200 ${hovered ? "opacity-100" : "opacity-0"}`}>
        ...
        <div className="absolute top-2 left-2 flex gap-1">
          {experiment.isFree && (
            <span className="px-1.5 py-0.5 bg-emerald-500 text-white text-[9px] font-bold rounded-full shadow">FREE</span>
          )}
          {experiment.isNew && (
            <span className="px-1.5 py-0.5 bg-orange-500 text-white text-[9px] font-bold rounded-full shadow animate-pulse">NEW</span>
          )}
          {!experiment.isFree && (
            <span className="px-1.5 py-0.5 bg-black/50 text-white text-[9px] rounded-full flex items-center gap-0.5 backdrop-blur-sm">
              <svg width="8" height="9" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="absolute top-2 right-2">
          <span className={`px-1.5 py-0.5 text-[9px] font-semibold rounded-full ${typeStyle.bg} ${typeStyle.text} opacity-95 shadow-sm`}>
            {experiment.type}
          </span>
        </div>
      </div>
      ...
      <div className="px-3 py-2.5">
        <h3 className={`text-[12.5px] font-semibold leading-snug line-clamp-2 transition-colors ${hovered ? "text-blue-600" : "text-gray-800"}`}>
          {experiment.title}
        </h3>
        <div className="flex items-center justify-between mt-2">
          <div className="flex items-center gap-1 text-gray-400">
            <EyeIcon />
            <span className="text-[10px]">{experiment.usageCount}</span>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="text-[9px] bg-blue-50 text-blue-600 hover:bg-blue-100 px-2 py-0.5 rounded-full font-semibold transition-colors"
            >
              Open →
            </button>
          </div>
